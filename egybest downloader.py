...
@click.option('--dest', '-d', default=expanduser("~")+os.sep+"Documents/movieslinks", help='Destination path.')
...
def main(search, quality, seasons, output_file, dest, executable_path, quiet):
    ...
    if not quiet: click.echo('opening a popup ad for 3 secs...')
    ...
    while cur<len(page_links):
        ...
        try:
            ...
            if chosen_result['u'].lower().startswith('movie'):
                ...
                movietime = browser.find_elements_by_xpath('//table[contains(@class, "movieTable")]//tbody//tr[6]//td[2]')
            
                click.echo(f'movie time is {movietime[0].text}')

                qualities_list = []
                for x in AVAILABLE_QUALITIES:
                    if browser.find_elements_by_xpath('//table[contains(@class, "dls_table")]//tbody//tr[td//text()[contains(., "{}p")]]//a[1]'.format(x)):
                        qualities_list.append(x)
                        size = browser.find_elements_by_xpath('//table[contains(@class, "dls_table")]//tbody//tr//td[contains(., "{}p")]//following::td'.format(x))
